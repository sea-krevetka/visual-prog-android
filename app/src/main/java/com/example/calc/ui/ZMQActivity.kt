...
class ZMQActivity : AppCompatActivity() {
    ...
    private fun startServer() {
        ...
        serverThread = Thread {
            ...
            try {
                ...
                appendLog("[SERVER] Error: ${t.message}")
            } finally {
    ...
    private fun startClient() {
        ...
        clientThread = Thread {
            ...
            try {
                ...
                appendLog("[CLIENT] Error: ${t.message}")
            } finally {
                try { socket?.close() } catch (ignored: Exception) {}
                try { context?.close() } catch (ignored: Exception) {}
    ...
    private fun stopAll() {
        ...
        serverRunning = false
        clientRunning = false
        try { serverThread?.join(2000) } catch (ignored: Exception) {}
        try { clientThread?.join(2000) } catch (ignored: Exception) {}
